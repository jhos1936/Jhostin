<?php
require_once dirname(__DIR__, 3) . '/app/controllers/GradoController.php';

$controller = new GradoController();
$grados = $controller->listar();
?>
...
<nav class="top-bar d-flex align-items-center justify-content-between flex-wrap gap-2">
    ...
    <div class="d-flex align-items-center flex-wrap gap-1">
        ...
        <a href="grados.php" class="nav-link active"><i class="bi bi-mortarboard me-1"></i>Grados</a>
...
<div class="main-container">
    <div class="crud-card">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="m-0" style="color: #0d0216; font-weight: 600;">Grados Registrados</h3>
            <button class="btn btn-purple px-4" data-bs-toggle="modal" data-bs-target="#modalGrado">
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    <?php if (!empty($grados)): ?>
                        ...
                        <tr>
                            <td><strong>#<?= htmlspecialchars($g['id_grado']) ?></strong></td>
                            <td><?= htmlspecialchars($g['nombre_grado']) ?></td>
                            <td><span class="badge bg-light text-dark border"><?= htmlspecialchars($g['seccion']) ?></span></td>
                            <td class="text-center">
                                <a href="procesar_grado.php?accion=eliminar&id=<?= $g['id_grado'] ?>"
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="4" class="text-center text-muted py-4">No hay grados registrados.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div class="modal fade" id="modalGrado" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 15px; border: none;">
            <form action="procesar_grado.php?accion=crear" method="POST">
                <div class="modal-header" style="background: #0d0216; color: #bc80ff; border-top-left-radius: 14px; border-top-right-radius: 14px;">
                    <h5 class="modal-title"><i class="bi bi-plus-circle me-2"></i>Registrar Nuevo Grado</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-bold">Nombre del Grado:</label>
                        <input type="text" name="nombre_grado" class="form-control" placeholder="Ej: 1ro Primaria" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold">Sección:</label>
                        <input type="text" name="seccion" class="form-control" placeholder="Ej: A">
                    </div>
                </div>
